...
class Tower(Pice):
    ...
    def move_up(self, moves, board):
        ...
        while next_posY >= 0 and board[next_posY][self.posX]['p'] == '':
            moves.append(Move((self.posX,self.posY),(self.posX,next_posY)))
            next_posY-=1
        if next_posY >= 0 and board[next_posY][self.posX]['p'].side != self.side:
            moves.append(Move((self.posX,self.posY),(self.posX,next_posY)))

    def move_left(self, moves, board):
        next_posX = self.posX-1
        while next_posX >= 0 and board[self.posY][next_posX]['p'] == '':
            moves.append(Move((self.posX,self.posY),(next_posX,self.posY)))
            next_posX-=1
        if next_posX >= 0 and board[self.posY][next_posX]['p'].side != self.side:
            moves.append(Move((self.posX,self.posY),(next_posX,self.posY)))
    ...
    def move_right(self, moves, board):
        ...
        while next_posX <= 7 and board[self.posY][next_posX]['p'] == '':
            moves.append(Move((self.posX,self.posY),(next_posX,self.posY)))
            next_posX+=1
        if next_posX <= 7 and board[self.posY][next_posX]['p'].side != self.side:
            moves.append(Move((self.posX,self.posY),(next_posX,self.posY)))
    ...
    def move_down(self, moves, board):
        ...
        while next_posY <= 7 and board[next_posY][self.posX]['p'] == '':
            moves.append(Move((self.posX,self.posY),(self.posX,next_posY)))
            next_posY+=1
        if next_posY <= 7 and board[next_posY][self.posX]['p'].side != self.side:
            moves.append(Move((self.posX,self.posY),(self.posX,next_posY)))